...
import { ConfigService, FileProvidersService, HostAppService, Platform, PlatformService } from 'tlink-core'
...
export class SSHService {
    ...
    async convertPrivateKeyFileToPuTTYFormat (profile: SSHProfile): Promise<{ passphrase: string|null, privateKeyFile: tmp.FileResult|null }> {
        ...
        for (const pk of profile.options.privateKeys) {
            ...
            const curPassphrase = await this.passwordStorage.loadPrivateKeyPassword(keyHash) ?? 'tlink'
