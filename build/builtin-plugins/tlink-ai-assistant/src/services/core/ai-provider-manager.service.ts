...
export class AiProviderManagerService implements ProviderManager {
    ...
    registerProvider(provider: BaseAiProvider, setAsActiveIfFirst: boolean = false): void {
        ...
        // Only auto-set as active if explicitly requested AND no active provider exists
        // This prevents overriding the configured default provider
        if (setAsActiveIfFirst && !this.activeProvider) {
            // Check if provider is enabled before setting as active
    ...
    getActiveProvider(): BaseAiProvider | undefined {
        ...
        const config = provider.getConfig();
        if (config?.enabled === false) {
            // Provider was disabled, try to switch to another enabled provider
            const enabledProviders = this.getEnabledProviders();
            if (enabledProviders.length > 0) {
                const fallbackProvider = enabledProviders[0];
                this.logger.warn(`Active provider ${this.activeProvider} is disabled, switching to ${fallbackProvider.name}`);
                this.setActiveProvider(fallbackProvider.name);
                return fallbackProvider;
            } else {
                this.logger.error('Active provider is disabled and no other enabled providers available');
                this.activeProvider = null;
                return undefined;
            }
        }
    ...
    setActiveProvider(name: string): boolean {
        ...
        if (provider) {
            const config = provider.getConfig();
            if (config?.enabled === false) {
                this.logger.warn(`Provider ${name} is disabled, cannot set as active`);
                
                // Try to find another enabled provider
                const enabledProviders = this.getEnabledProviders();
                if (enabledProviders.length > 0) {
                    const fallbackProvider = enabledProviders[0];
                    this.logger.info(`Switching to enabled provider: ${fallbackProvider.name}`);
                    return this.setActiveProvider(fallbackProvider.name);
                } else {
