...
export class AiSettingsTabComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        ...
        // Listen to config changes to update current provider display
        this.config.onConfigChange().pipe(
            takeUntil(this.destroy$)
        ).subscribe(change => {
            if (change.key === 'defaultProvider' || change.key === '*') {
                this.loadSettings(); // Reload to get updated provider
            }
        });
        ...
        // Avoid ExpressionChanged errors when labels normalize after migration
    ...
    private loadSettings(): void {
        ...
        // Ensure the provider name is displayed correctly
        if (this.currentProvider) {
            const providerConfig = this.config.getProviderConfig(this.currentProvider);
            if (providerConfig?.displayName) {
                // Use display name if available, but keep the internal name for switching
                this.logger.debug('Current provider', { 
    ...
    switchProvider(providerName: string): void {
        if (this.aiService.switchProvider(providerName)) {
            ...
            this.loadSettings(); // Reload to sync with config
            this.loadProviderStatus();
    ...
    getCurrentProviderDisplayName(): string {
        if (!this.currentProvider) {
            return this.t.common.notConfigured || 'Not Configured';
        }
        ...
        // Fallback to provider name mapping
        ...
        return providerNames[this.currentProvider] || this.currentProvider;
    }
