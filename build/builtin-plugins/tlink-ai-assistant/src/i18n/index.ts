...
export class TranslateService {
    private currentLang$ = new BehaviorSubject<SupportedLanguage>('en-US');
    private currentTranslation$ = new BehaviorSubject<TranslationKeys>(enUS);

    constructor(private config: ConfigProviderService) {
        // Load saved language settings
        const savedLang = this.config.get<string>('language', 'en-US') as SupportedLanguage;
        this.setLanguage(savedLang || 'en-US');
    }
