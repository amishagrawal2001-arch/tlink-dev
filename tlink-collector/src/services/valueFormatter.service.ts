...
/**
 * Type-aware formatter for collector sample values.
 *
 * Same shape + heuristics as the gNMI plugin's formatter — kept as its
 * own service so the collector plugin has no cross-plugin runtime dep.
 * Recognized leaf patterns:
 ...
 *   - `utilization`, `percent`, `usage`, `cpu_percent` on a 0-100 value →
 *     percentage with `%` unit suffix.
 *   - `octets`, `bytes`, `memory_bytes`, `packets` on any integer →
 *     thousands separator or byte scaling for readability.
 */
@Injectable({ providedIn: 'root' })
export class CollectorValueFormatterService {
    /**
    ...
    private looksLikeByteValue (pathLower: string, leaf: string): boolean {
        ...
        return leaf === 'used' || leaf === 'free' || leaf === 'available' ||
               ...
               // Prometheus / Telegraf convention — memory_bytes, disk_bytes,
               // node_filesystem_size_bytes. Matches the "_bytes" suffix that
               // the OpenMetrics naming guide recommends for all byte-count
               // gauges.
               leaf.endsWith('_bytes')
    }
    ...
    private looksLikePercentLeaf (leaf: string): boolean {
        return leaf === 'utilization' || leaf === 'percent' ||
               ...
               // Prometheus / Telegraf convention — cpu_percent,
               // memory_used_percent, disk_usage_percent all end in _percent
               // (underscore) rather than the OpenConfig dash-suffix form.
               leaf.endsWith('_percent') ||
    ...
    private looksLikeCounterLeaf (leaf: string): boolean {
        return leaf.endsWith('-octets') || leaf.endsWith('-bytes') ||
               ...
               // Prometheus counter convention — requests_total, errors_total,
               // http_requests_total etc. Applies thousands separators on
               // large counter reads even in "Value" chart mode.
               leaf.endsWith('_total')
    }
...
/**
 * Return shape from CollectorValueFormatterService.format(). Split value +
