...
export class ThemeService implements OnDestroy {
    ...
    private tlinkSubscription?: Subscription;
    ...
    private init(): void {
        ...
        // Listen to Tlink theme changes (with debounce)
        this.tlinkSubscription = this.tlinkConfig.changed$.pipe(
            debounceTime(100)
        ).subscribe(() => {
    ...
    applyTheme(theme: ThemeType): void {
        ...
        if (theme === 'auto') {
            effectiveTheme = this.getTlinkEffectiveTheme();
        }
...
${this.containerSelectors.join(',\n')} {
    ...
    /**
     * Get Tlink's current effective theme
     */
    private getTlinkEffectiveTheme(): 'light' | 'dark' {
        const appearance = this.tlinkConfig.store?.appearance;
    ...
    ngOnDestroy(): void {
        ...
        this.tlinkSubscription?.unsubscribe();
