...
import { Logger, LogService, PlatformService, BOOTSTRAP_DATA, BootstrapData, PluginInfo } from 'tlink-core'
...
export class PluginManagerService {
    ...
    listAvailable (query?: string): Observable<PluginInfo[]> {
        return forkJoin(
            this._listAvailableInternal('tlink-', 'tlink-plugin', query),
    ...
    _listAvailableInternal (namePrefix: string, keyword: string, query?: string): Observable<PluginInfo[]> {
        return from(
            ...
            map(response => response.data.objects
                .filter(item => !item.keywords?.includes('tlink-dummy-transition-plugin'))
                .map(item => ({
